...
if(isset($_GET['ref']))	{
	...
	$stmt->execute(array('KEY_ID' => $ref, 'TYPE' => 'ckd'));
	$row = $stmt->fetch(\PDO::FETCH_OBJ);
	if(!$row){
		echo "<script>window.location.href = 'ckd.php';</script>";
	}
}
else echo "<script>window.location.href = 'ckd.php';</script>";
?>
...
<div class="topbar">
    <div class="breadcrumbs">
        ...
        <a href="ckd.php"><span>CKD Parts Enter</span></a>
        <i class="fa fa-caret-right" aria-hidden="true"></i>
        <a href="#"><span>CKD Parts Enter Details</span></a>
    </div> <!-- /breadcrumbs -->
    <div class="signin">
        <span>Hey, Admin</span>
    </div> <!-- /signin -->
...
<div class="content">
	<h1 class="page-title">
		<span class="title">CKD Parts Enter Details</span>
		...
		<a href="<?php echo $base_url;?>/views/forms/ckd-edit.php?ref=<?php echo $ref;?>" class="btn btn-primary btn-sm pull-right">
	...
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 detail-info no-padding">
		<h3>
			...
			<button class="export-file btn btn-primary btn-sm pull-right" data-action="print" data-entries="entries" data-parts = "parts" data-key="<?php echo $row->KEY_ID;?>" data-file-name = "ckd_enter" style="margin-left: 5px;">Print</button>
			<button class="export-file btn btn-primary btn-sm pull-right" data-action="export-excel" data-entries="entries" data-parts = "parts" data-key="<?php echo $row->KEY_ID;?>" data-file-name = "ckd_enter" style="margin-left: 5px;">Get Excel</button>
		</h3>
	...
	<div class="table-responsive col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding">
		...
		<table class="table table-bordered table-striped searchable-table editable-table">
			...
			<tbody>
				<?php 
				$stmt = $con->prepare("SELECT * FROM parts WHERE ENTRY_REFERENCE_ID = :ENTRY_REFERENCE_ID ORDER BY KEY_ID ASC");
				...
				foreach ($rows as $r) {
					...
					<tr class='show-row'>
						...
						<td class='fixed-width'>
							<a href='#' data-type='ckd' data-key='$r->KEY_ID' class='inline-edit btn btn-default'><i class='fa fa-pencil'></i></a>
							<a href='$base_url/functions/process-forms.php?del=execute&param=KEY_ID&target=$r->KEY_ID&entity=parts&page=display/ckd-details.php?ref=$ref' class='btn btn-default' onclick=\"return confirm('Are you sure that you want to delete this record?');\"><i class = 'fa fa-trash'></i></a>
						</td>
					</tr>";
				}
				?>
	...
	<div class="content-modal-body">
		...
		<form action="<?php echo $base_url;?>/functions/process-forms.php" method="post" enctype="multipart/form-data" class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding">
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form-input no-padding">
				<input type="hidden" name="type" value="ckd">
