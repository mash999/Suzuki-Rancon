...
if(isset($_GET['ref']))	{
	...
	$stmt->execute(array('KEY_ID' => $ref, 'TYPE' => 'manufacturing-parts'));
	$row = $stmt->fetch(\PDO::FETCH_OBJ);
	if(!$row){
		echo "<script>window.location.href = 'manufacturing-parts.php';</script>";
	}
}
else echo "<script>window.location.href = 'manufacturing-parts.php';</script>";
?>
...
<div class="topbar">
    <div class="breadcrumbs">
        ...
        <a href="manufacturing-parts.php"><span>Manufacturing Parts Enter</span></a>
        <i class="fa fa-caret-right" aria-hidden="true"></i>
        <a href="#"><span>Manufacturing Parts Enter Details</span></a>
    </div> <!-- /breadcrumbs -->
    <div class="signin">
        <span>Hey, Admin</span>
    </div> <!-- /signin -->
...
<div class="content">
	<h1 class="page-title">
		<span class="title">Manufacturing Parts Enter Details</span>
		...
		<a href="<?php echo $base_url;?>/views/forms/manufacturing-parts-edit.php?ref=<?php echo $ref;?>" class="btn btn-primary btn-sm pull-right">
			<i class="fa fa-pencil"></i>&nbsp; Edit Records
		</a>
		<?php } ?>
		<label for="search-input"><i class="fa fa-search"></i></label>
		<input type="text" id="search-input" class="form-control" placeholder="Search Manufacturing Parts">
	</h1>
	...
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 detail-info no-padding">
		<h3>
			...
			<button class="export-file btn btn-primary btn-sm pull-right" data-action="print" data-entries="entries" data-parts = "parts" data-key="<?php echo $row->KEY_ID;?>" data-file-name = "manufacture_enter" style="margin-left: 5px;">Print</button>
			<button class="export-file btn btn-primary btn-sm pull-right" data-action="export-excel" data-entries="entries" data-parts = "parts" data-key="<?php echo $row->KEY_ID;?>" data-file-name = "manufacture_enter" style="margin-left: 5px;">Get Excel</button>
		</h3>
		...
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 table-responsive info-table no-padding-left">
			<table class="table table-bordered">
				<tr>
					...
					<td>
						<?php 
						if ($row->REQUISITION_NUMBER == 0)	$requisition_number =  "----";
						else $requisition_number = $row->REQUISITION_NUMBER; 
						?>
						<span class="cell-value"><?php echo $requisition_number;?></span>
						<input type="text" class="edit-basic-input" name="requisition-number">
					</td>
				</tr>
				<tr>
				...
				<tr class="hidden">
					...
					<td>
						<span class="cell-value"><?php echo $row->SUPPLIER_CHALLAN_NUMBER;?></span>
						<input type="text" class="edit-basic-input">
					</td>
				</tr>
				<tr>
				...
				foreach ($rows as $r) {
					...
					<tr class='show-row'>
						...
						<td class='fixed-width'>
							<a href='#' data-type='manufacturing-parts' data-key='$r->KEY_ID' class='inline-edit btn btn-default'><i class='fa fa-pencil'></i></a>
							<a href='$base_url/functions/process-forms.php?del=execute&param=KEY_ID&target=$r->KEY_ID&entity=parts&page=display/manufacturing-parts-details.php?ref=$ref' class='btn btn-default' onclick=\"return confirm('Are you sure that you want to delete this record?');\"><i class = 'fa fa-trash'></i></a>
						</td>
					</tr>";
				}
				?>
...
<div id="parts-modal" class="content-modal">
	...
	<div class="content-modal-body">
		<h3>Enter Manufacturing Parts</h3>
		<h4>
