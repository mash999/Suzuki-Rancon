...
<div class="content">
	<h1 class="page-title">
		<span class="title">Claims for CKD BOM</span>
		<a href="<?php echo $base_url;?>/views/forms/ckd-bom-claim-enter.php" class="btn btn-primary btn-sm pull-right">
	...
	<div class="table-responsive col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding">
		<table class="table table-bordered table-striped searchable-table">
			...
			<tbody>
				<?php 
				$stmt = $con->query("SELECT * FROM claims WHERE TYPE = 'ckd-bom-claim' ORDER BY KEY_ID DESC");
				...
				foreach ($ckds as $c) {
					...
					<tr class='$class_name' id='row-num-$i'>
						...
						<td class='fixed-width'>
							<a href='$base_url/views/display/ckd-bom-claim-details.php?ref=$c->KEY_ID' class='btn btn-primary btn-sm'>Details</a>
							<button class='export-file btn btn-primary btn-sm' data-action='export-excel' data-entries='claims' data-parts = 'claims_parts' data-key='$c->KEY_ID' data-file-name = 'ckd_bom_claims'>Get Excel</button>
							<button class='export-file btn btn-primary btn-sm' data-action='print' data-entries='claims' data-parts = 'claims_parts' data-key='$c->KEY_ID' data-file-name = 'ckd_bom_claims'>Print</button>
						</td>
					</tr>";
				}
				?>
...
<div id="export-modal">
	<div class="export-modal-body">
		...
		<form action="<?php echo $base_url;?>/functions/export-file.php" method="post" target="blank">
			<input type="hidden" name="file-name" value="CKDBOM_CLAIMS_">
			<div class="left">
