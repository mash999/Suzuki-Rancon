...
if(isset($_GET['ref']))	{
	...
	$stmt->execute(array('KEY_ID' => $ref, 'TYPE' => 'manufacturing-claim'));
	$row = $stmt->fetch(\PDO::FETCH_OBJ);
	if(!$row){
		echo "<script>window.location.href = 'manufacturing-claim.php';</script>";
	}
}
else echo "<script>window.location.href = 'manufacturing-claim.php';</script>";
?>
...
<div class="topbar">
    <div class="breadcrumbs">
        ...
        <a href="manufacturing-claim.php"><span>Claims for Manufacturing Parts</span></a>
        <i class="fa fa-caret-right" aria-hidden="true"></i>
        <a href="#"><span>Claims for Manufacturing Parts Details</span></a>
    </div> <!-- /breadcrumbs -->
    <div class="signin">
        <span>Hey, Admin</span>
    </div> <!-- /signin -->
...
<div class="content">
	<h1 class="page-title">
		<span class="title">Claims for Manufacturing Parts Details</span>
		...
		<a href="<?php echo $base_url;?>/views/forms/manufacturing-claim-edit.php?ref=<?php echo $ref;?>" class="btn btn-primary btn-sm pull-right">
	...
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 detail-info no-padding">
		<h3>
			...
			<button class="export-file btn btn-primary btn-sm pull-right" data-action="print" data-entries="claims" data-parts = "claims_parts" data-key="<?php echo $row->KEY_ID;?>" data-file-name = "manufacture_claims" style="margin-left: 5px;">Print</button>
			<button class="export-file btn btn-primary btn-sm pull-right" data-action="export-excel" data-entries="claims" data-parts = "claims_parts" data-key="<?php echo $row->KEY_ID;?>" data-file-name = "manufacture_claims">Get Excel</button>
		</h3>
				...
				foreach ($rows as $r) {
					...
					<tr class='show-row'>
						...
						<td class='fixed-width'>
							...
							<a href='$base_url/functions/process-forms.php?del=execute&param=KEY_ID&target=$r->KEY_ID&entity=claims_parts&page=display/manufacturing-claim-details.php?ref=$ref' class='btn btn-default' onclick=\"return confirm('Are you sure that you want to delete this record?');\"><i class = 'fa fa-trash'></i></a>
						</td>
					</tr>";
				}
				?>
...
<div id="parts-modal" class="content-modal">
	...
	<div class="content-modal-body">
		<h3>Manufacturing Parts Claims</h3>
		<h4>
		...
		<form action="<?php echo $base_url;?>/functions/process-forms.php" method="post" enctype="multipart/form-data" class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding">
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form-input no-padding">
				<input type="hidden" name="type" value="manufacturing-claim">
