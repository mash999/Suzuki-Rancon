...
if(isset($_GET['ref']))	{
	...
	$stmt->execute(array('KEY_ID' => $ref, 'TYPE' => 'manufacturing-issue'));
	$row = $stmt->fetch(\PDO::FETCH_OBJ);
	if(!$row){
		echo "<script>window.location.href = 'manufacturing-issue.php';</script>";
	}
}
else echo "<script>window.location.href = 'manufacturing-issue.php';</script>";
?>
...
<div class="topbar">
    <div class="breadcrumbs">
        ...
        <a href="manufacturing-issue.php"><span>Manufacturing Parts Issue to Assembly</span></a>
        <i class="fa fa-caret-right" aria-hidden="true"></i>
        <a href="#"><span>Manufacturing Parts Issue to Assembly Details</span></a>
    </div> <!-- /breadcrumbs -->
    <div class="signin">
        <span>Hey, Admin</span>
    </div> <!-- /signin -->
...
<div class="content">
	<h1 class="page-title">
		<span class="title">Manufacturing Parts Issue to Assembly Details</span>
		...
		<a href="<?php echo $base_url;?>/views/forms/manufacturing-issue-edit.php?ref=<?php echo $ref;?>" class="btn btn-primary btn-sm pull-right">
	...
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 detail-info no-padding">
		<h3>
			...
			<button class="export-file btn btn-primary btn-sm pull-right" data-action="print" data-entries="issues" data-parts = "issue_records" data-key="<?php echo $row->KEY_ID;?>" data-file-name = "manufacture_issue" style="margin-left: 5px;">Print</button>
			<button class="export-file btn btn-primary btn-sm pull-right" data-action="export-excel" data-entries="issues" data-parts = "issue_records" data-key="<?php echo $row->KEY_ID;?>" data-file-name = "manufacture_issue" style="margin-left: 5px;">Get Excel</button>
		</h3>
				...
				foreach ($rows as $r) {
					...
					<tr class='show-row'>
						...
						<td class='fixed-width'>
							<a href='#' data-type='manufacturing-issue' data-key='$r->KEY_ID' class='inline-edit btn btn-default'><i class='fa fa-pencil'></i></a>
							<a href='$base_url/functions/process-forms.php?del=execute&param=KEY_ID&target=$r->KEY_ID&entity=issue_records&page=display/manufacturing-issue-details.php?ref=$ref' class='btn btn-default' onclick=\"return confirm('Are you sure that you want to delete this record?');\"><i class = 'fa fa-trash'></i></a>
						</td>
					</tr>";
				}
				?>
...
<div id="parts-modal" class="content-modal">
	...
	<div class="content-modal-body">
		<h3>Manufacturing Part Issues</h3>
		<h4>
			Requisition Number -<?php echo $row->KEY_ID;?>
		</h4>
				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<td><input type="text" name="frame-number[]" placeholder="Frame Number"></td>
