...
if(isset($_GET['ref']))	{
	...
	$stmt->execute(array('KEY_ID' => $ref, 'TYPE' => 'spare-claim'));
	$row = $stmt->fetch(\PDO::FETCH_OBJ);
	if(!$row){
		echo "<script>window.location.href = 'spare-claim.php';</script>";
	}
}
else echo "<script>window.location.href = 'spare-claim.php';</script>";
?>
...
<div class="topbar">
    <div class="breadcrumbs">
        ...
        <a href="spare-claim.php"><span>Claims for Spare Parts</span></a>
        <i class="fa fa-caret-right" aria-hidden="true"></i>
        <a href="#"><span>Claims for Spare Parts Details</span></a>
    </div> <!-- /breadcrumbs -->
    <div class="signin">
        <span>Hey, Admin</span>
    </div> <!-- /signin -->
...
<div class="content">
	<h1 class="page-title">
		<span class="title">Claims for Spare Parts Details</span>
		...
		<a href="<?php echo $base_url;?>/views/forms/spare-claim-edit.php?ref=<?php echo $ref;?>" class="btn btn-primary btn-sm pull-right">
				...
				foreach ($rows as $r) {
					...
					<tr class='show-row'>
						...
						<td class='fixed-width'>
							...
							<a href='$base_url/functions/process-forms.php?del=execute&param=KEY_ID&target=$r->KEY_ID&entity=claims_parts&page=display/spare-claim-details.php?ref=$ref' class='btn btn-default' onclick=\"return confirm('Are you sure that you want to delete this record?');\"><i class = 'fa fa-trash'></i></a>
						</td>
					</tr>";
				}
				?>
	...
	<div class="content-modal-body">
		...
		<form action="<?php echo $base_url;?>/functions/process-forms.php" method="post" enctype="multipart/form-data" class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding">
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form-input no-padding">
				<input type="hidden" name="type" value="spare-claim">
