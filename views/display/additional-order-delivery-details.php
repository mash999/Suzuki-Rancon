...
if(isset($_GET['ref']))	{
	...
	$stmt = $con->prepare("SELECT * FROM additional_delivery WHERE KEY_ID = :KEY_ID");
	$stmt->execute(array('KEY_ID' => $ref));
	$row = $stmt->fetch(\PDO::FETCH_OBJ);
	if(!$row){
		echo "<script>window.location.href = 'additional-order-delivery.php';</script>";
	}
}
else echo "<script>window.location.href = 'additional-order-delivery.php';</script>";
?>
...
<div class="topbar">
    <div class="breadcrumbs">
        ...
        <a href="additional-order-delivery.php">Additional Parts Delivery</a>
        <i class="fa fa-caret-right" aria-hidden="true"></i>
        <a href="#">Additional Parts Delivery Details</a>
    </div> <!-- /breadcrumbs -->
    <div class="signin">
        <span>Hey, Admin</span>
    </div> <!-- /signin -->
...
<div class="content">
	<h1 class="page-title">
		<span class="title">Additional Parts Delivery Details</span>
		...
		<a href="<?php echo $base_url;?>/views/forms/additional-order-delivery-edit.php?ref=<?php echo $ref;?>" class="btn btn-primary btn-sm pull-right">
	...
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 detail-info no-padding">
		<h3>
			Additional Delivery Number - <?php echo $ref;?></span>
			<?php if($_SESSION['rancon_access_level'] > 1){ ?>  
			<a href="#" class="btn btn-primary btn-sm pull-right delete-records" data-type="additional" data-main="additional_delivery" data-section="delivery_parts" data-ref="<?php echo $ref;?>" style="margin-left: 5px;">Delete</a> 
			<span data-key="<?php echo $ref;?>" data-section="additional-delivery" class="btn btn-primary btn-sm pull-right activate-edit" style="margin-left: 5px;">Edit</span>
			<?php } ?>

			<button class="export-file btn btn-primary btn-sm pull-right" data-action="print" data-entries="additional_delivery" data-parts = "delivery_parts" data-key="<?php echo $row->KEY_ID;?>" data-file-name = "additional_delivery" style="margin-left: 5px;">Print</button>
			<button class="export-file btn btn-primary btn-sm pull-right" data-action="export-excel" data-entries="additional_delivery" data-parts = "delivery_parts" data-key="<?php echo $row->KEY_ID;?>" data-file-name = "additional_delivery" style="margin-left: 5px;">Get Excel</button>
		</h3>
				...
				foreach ($rows as $r) {
					...
					<tr class='show-row'>
						...
						<td class='fixed-width'>
							<a href='#' data-type='additional-delivery' data-key='$r->KEY_ID' class='inline-edit btn btn-default'><i class='fa fa-pencil'></i></a>
							<a href='$base_url/functions/process-forms.php?del=execute&param=KEY_ID&target=$r->KEY_ID&entity=delivery_parts&page=display/additional-order-delivery-details.php?ref=$ref' class='btn btn-default' onclick=\"return confirm('Are you sure that you want to delete this record?');\"><i class = 'fa fa-trash'></i></a>
						</td>
					</tr>";
				}
				?>
...
<div id="parts-modal" class="content-modal">
	...
	<div class="content-modal-body">
		...
		<h4>
			Additional Delivery Number  - <?php echo $ref;?>
		</h4>
		...
		<form action="<?php echo $base_url;?>/functions/process-forms.php" method="post" enctype="multipart/form-data" class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padding">
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form-input no-padding">
				...
				<input type="hidden" name="delivery-type" value="additional">
